...
def fecha_estimada_llegada(meses_desde_hoy: int) -> str:
    ...
    hoy = date.today()
    mes_total = hoy.month - 1 + int(meses_desde_hoy)
...
with col_form:
    with st.form("nuevo_objetivo", clear_on_submit=True):
        ...
        costo_total = col_costo.number_input("Costo Total (hoy)", min_value=0.0, step=1000.0)
        ahorro_previo = st.number_input("Ahorrado hoy (misma moneda)", min_value=0.0, step=1000.0)
...
if st.session_state.objetivos:
    ...
    if st.button("Eliminar metas seleccionadas"):
        st.session_state.objetivos = [
...
if objetivos_enriquecidos:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # MONITOR KANBAN — tarjetas por estado con reordenamiento drag & drop
    # ══════════════════════════════════════════════════════════════════════════

    # Construir JSON de datos para el componente HTML
    import json as _json

    _cards_data = []
    for _o in objetivos_enriquecidos:
        _m = _o['moneda_meta']
        _plazo_orig = int(_o.get("Plazo (Meses)", 0))
        _meses_reales = meses_para_acumular(
            _o['faltante_futuro'], _o['cuota_asignada_meta'], _o['r_mensual']
        )
        _fecha_display = fecha_estimada_llegada(_plazo_orig) if _o['estado'] == "En curso" else (
            fecha_estimada_llegada(_meses_reales) if _meses_reales else "—"
        )
        _desfase = (_meses_reales - _plazo_orig) if (_meses_reales and _o['estado'] != "En curso") else 0
        _pct_prog = min(100, round(float(_o["Ya Ahorrado"]) / float(_o["Costo Total"]) * 100, 1)) if float(_o["Costo Total"]) > 0 else 0
        _diff = _o['cuota_asignada_meta'] - _o['cuota_ideal_meta']
        _instr = _o['instrumento']
        # Alternativas del instrumento
        _alts_str = " · ".join(_instr.get("alternativas", [])) if _instr.get("alternativas") else ""
        # Tooltips relevantes
        _terminos = [k for k in TOOLTIPS_INSTRUMENTOS if k.lower() in _instr['tipo'].lower()]
        _terminos += [k for k in TOOLTIPS_INSTRUMENTOS
                      for alt in _instr.get("alternativas", [])
                      if k.lower() in alt.lower()]
        _terminos = list(dict.fromkeys(_terminos))[:2]
        _tooltips_list = [{"term": t, "desc": TOOLTIPS_INSTRUMENTOS[t]} for t in _terminos]

        _cards_data.append({
            "id": _o["Meta"],
            "meta": _o["Meta"],
            "categoria": _o.get("Categoría", "Otro"),
            "prioridad": _o.get("Prioridad", "Media"),
            "moneda": _m,
            "pct_prog": _pct_prog,
            "ya_ahorrado": float(_o["Ya Ahorrado"]),
            "costo_total": float(_o["Costo Total"]),
            "costo_futuro": round(_o["costo_futuro"], 2),
            "faltante_futuro": round(_o["faltante_futuro"], 2),
            "cuota_ideal": round(_o["cuota_ideal_meta"], 2),
            "cuota_asignada": round(_o["cuota_asignada_meta"], 2),
            "diff_cuota": round(_diff, 2),
            "estado": _o['estado'],
            "fecha_llegada": _fecha_display,
            "desfase_meses": _desfase,
            "plazo_orig": _plazo_orig,
            "instrumento_tipo": _instr['tipo'],
            "instrumento_emoji": _instr['emoji'],
            "instrumento_desc": _instr['descripcion'],
            "alternativas": _alts_str,
            "tooltips": _tooltips_list,
        })

    _cards_json = _json.dumps(_cards_data, ensure_ascii=False)
    _ahorro_disp = ahorro_dispuesto
    _ahorro_asig = sum(
        convertir(_o['cuota_asignada_meta'], _o['moneda_meta'], moneda, tipos_cambio)
        for _o in objetivos_enriquecidos
    )
    _ahorro_libre = max(0.0, _ahorro_disp - _ahorro_asig)
    _metas_en_curso = sum(1 for _o in objetivos_enriquecidos if _o['estado'] == "En curso")

    # ── Instrucciones de reordenamiento ──────────────────────────────────────
    st.caption(
        "💡 **Arrastrá las tarjetas** para reordenar la prioridad de tus metas. "
        "El orden determina cómo se distribuye el ahorro en la cascada."
    )

    _kanban_html = f"""
<style>
  .km-wrap * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  .km-wrap {{ font-family: system-ui, -apple-system, sans-serif; width: 100%; }}

  /* Summary bar */
  .km-summary {{
    display: flex; gap: 0; margin-bottom: 14px;
    border: 1px solid rgba(128,128,128,0.18); border-radius: 10px; overflow: hidden;
  }}
  .km-sum-item {{
    flex: 1; padding: 10px 14px; background: rgba(128,128,128,0.05);
    border-right: 1px solid rgba(128,128,128,0.12);
  }}
  .km-sum-item:last-child {{ border-right: none; }}
  .km-sum-val {{ font-size: 15px; font-weight: 600; color: #e2e2e2; line-height: 1.2; }}
  .km-sum-lbl {{ font-size: 10px; color: #888; margin-top: 2px; text-transform: uppercase; letter-spacing: 0.05em; }}
  .km-sum-val.green {{ color: #4ade80; }}
  .km-sum-val.blue {{ color: #60a5fa; }}

  /* Columns */
  .km-cols {{ display: flex; gap: 10px; align-items: flex-start; }}
  .km-col {{ flex: 1; min-width: 0; }}
  .km-col-header {{
    padding: 6px 10px 6px 12px; border-radius: 8px; margin-bottom: 8px;
    display: flex; align-items: center; justify-content: space-between;
    font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em;
  }}
  .km-col-header.green {{ background: rgba(74,222,128,0.12); color: #4ade80; }}
  .km-col-header.yellow {{ background: rgba(251,191,36,0.12); color: #fbbf24; }}
  .km-col-header.red {{ background: rgba(248,113,113,0.12); color: #f87171; }}
  .km-col-badge {{ font-size: 11px; font-weight: 500; opacity: 0.7; }}
  ...
  .km-card:active {{ cursor: grabbing; }}
  .km-card.dragging {{ opacity: 0.4; transform: scale(0.98); }}
  .km-card.drag-over-card {{ border-color: rgba(128,128,128,0.5); }}

  .km-card-top {{ display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px; }}
  .km-card-title {{ font-size: 13px; font-weight: 600; color: #e2e2e2; line-height: 1.3; }}
  .km-card-sub {{ font-size: 10px; color: #666; margin-top: 2px; }}
  .km-prio-dot {{ width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; margin-top: 3px; }}

  /* Progress bar */
  .km-bar-wrap {{ background: rgba(128,128,128,0.15); border-radius: 4px; height: 5px; margin-bottom: 5px; overflow: hidden; }}
  .km-bar-fill {{ height: 100%; border-radius: 4px; transition: width 0.4s; }}
  .km-bar-labels {{ display: flex; justify-content: space-between; font-size: 10px; color: #555; margin-bottom: 9px; }}

  /* Main stats */
  .km-stat-grid {{ display: flex; gap: 8px; margin-bottom: 9px; }}
  .km-stat {{
    flex: 1; background: rgba(128,128,128,0.07); border-radius: 7px; padding: 7px 9px;
    border: 1px solid rgba(128,128,128,0.1);
  }}
  .km-stat-lbl {{ font-size: 10px; color: #666; margin-bottom: 2px; }}
  .km-stat-val {{ font-size: 13px; font-weight: 600; }}

  /* Date row */
  .km-date-row {{
    display: flex; align-items: center; justify-content: space-between;
    padding: 6px 9px; background: rgba(128,128,128,0.06);
    border-radius: 7px; margin-bottom: 9px;
    border: 1px solid rgba(128,128,128,0.1);
  }}
  .km-date-left {{ font-size: 10px; color: #666; }}
  .km-date-val {{ font-size: 12px; font-weight: 600; color: #e2e2e2; }}
  .km-desfase {{ font-size: 10px; color: #f87171; margin-top: 1px; text-align: right; }}
  ...
  .km-alert.yellow {{ background: rgba(251,191,36,0.1); color: #fbbf24; border: 1px solid rgba(251,191,36,0.2); }}
  .km-alert.red {{ background: rgba(248,113,113,0.1); color: #f87171; border: 1px solid rgba(248,113,113,0.2); }}

  /* Instrument accordion */
  .km-instr-toggle {{
    width: 100%; background: none; border: 1px solid rgba(128,128,128,0.15);
    border-radius: 7px; padding: 6px 10px;
    display: flex; justify-content: space-between; align-items: center;
    cursor: pointer; color: #888; font-size: 11px;
    transition: background 0.15s;
  }}
  .km-instr-toggle:hover {{ background: rgba(128,128,128,0.08); }}
  .km-instr-toggle .arrow {{ transition: transform 0.2s; font-size: 10px; }}
  .km-instr-toggle.open .arrow {{ transform: rotate(180deg); }}
  .km-instr-body {{
    display: none; border: 1px solid rgba(128,128,128,0.12);
    border-top: none; border-radius: 0 0 7px 7px;
    padding: 9px 10px; background: rgba(128,128,128,0.04);
  }}
  .km-instr-body.open {{ display: block; }}
  .km-instr-name {{ font-size: 12px; font-weight: 600; color: #c4c4c4; margin-bottom: 4px; }}
  .km-instr-desc {{ font-size: 11px; color: #777; line-height: 1.5; margin-bottom: 6px; }}
  .km-instr-alts {{ font-size: 10px; color: #555; margin-bottom: 6px; }}
  .km-tooltip-item {{ margin-top: 6px; border-top: 1px solid rgba(128,128,128,0.1); padding-top: 6px; }}
  .km-tooltip-term {{ font-size: 11px; font-weight: 600; color: #a0a0a0; }}
  .km-tooltip-def {{ font-size: 10px; color: #666; line-height: 1.5; margin-top: 2px; }}

  /* Drag order number */
  .km-order-num {{
    font-size: 10px; color: #444; font-weight: 600;
    width: 18px; height: 18px; border-radius: 50%;
    background: rgba(128,128,128,0.12); display: inline-flex;
...
<div class="km-wrap">
  ...
  <div class="km-summary">
    <div class="km-sum-item">
      <div class="km-sum-val green">{moneda} {_ahorro_disp:,.0f}</div>
      <div class="km-sum-lbl">Ahorro disponible / mes</div>
    </div>
    <div class="km-sum-item">
      <div class="km-sum-val blue">{moneda} {_ahorro_asig:,.0f}</div>
      <div class="km-sum-lbl">Cuotas asignadas</div>
    </div>
    <div class="km-sum-item">
      <div class="km-sum-val {'green' if _ahorro_libre > 0 else ''}">{moneda} {_ahorro_libre:,.0f}</div>
      <div class="km-sum-lbl">Remanente libre</div>
    </div>
    <div class="km-sum-item">
      <div class="km-sum-val">{_metas_en_curso} / {len(objetivos_enriquecidos)}</div>
      <div class="km-sum-lbl">Metas en curso</div>
    </div>
  </div>

  <div class="km-cols">
    <div class="km-col" id="col-encurso">
      <div class="km-col-header green">
        <span>En curso</span>
        <span class="km-col-badge" id="cnt-encurso">0</span>
      </div>
      <div class="km-dropzone" id="drop-encurso" data-estado="En curso"></div>
    </div>
    <div class="km-col" id="col-parcial">
      <div class="km-col-header yellow">
        <span>Cuota parcial</span>
        <span class="km-col-badge" id="cnt-parcial">0</span>
      </div>
      <div class="km-dropzone" id="drop-parcial" data-estado="Parcial"></div>
    </div>
    <div class="km-col" id="col-espera">
      <div class="km-col-header red">
        <span>Sin asignación</span>
        <span class="km-col-badge" id="cnt-espera">0</span>
      </div>
      <div class="km-dropzone" id="drop-espera" data-estado="En espera"></div>
    </div>
  </div>
...
(function() {{
  ...
  const PRIO_COLOR = {{ "Alta": "#f87171", "Media": "#fbbf24", "Baja": "#60a5fa" }};
  const BAR_COLOR  = {{ "En curso": "#4ade80", "Parcial": "#fbbf24", "En espera": "#f87171" }};
  const DROP_IDS   = {{ "En curso": "drop-encurso", "Parcial": "drop-parcial", "En espera": "drop-espera" }};
  const CNT_IDS    = {{ "En curso": "cnt-encurso", "Parcial": "cnt-parcial", "En espera": "cnt-espera" }};

  function fmtNum(n, moneda) {{
    return moneda + " " + Number(n).toLocaleString("es-AR", {{minimumFractionDigits:0, maximumFractionDigits:0}});
  }}

  function buildCard(c, orderNum) {{
    const estado = c.estado;
    const barColor = BAR_COLOR[estado] || "#888";
    const prioColor = PRIO_COLOR[c.prioridad] || "#888";
    const diffColor = c.diff_cuota >= 0 ? "#4ade80" : "#f87171";
    const diffTxt = c.diff_cuota >= 0
      ? "+" + fmtNum(c.diff_cuota, c.moneda)
      : "-" + fmtNum(Math.abs(c.diff_cuota), c.moneda);

    // Build tooltips HTML
    let tooltipsHtml = "";
    if (c.tooltips && c.tooltips.length > 0) {{
      tooltipsHtml = c.tooltips.map(t =>
        `<div class="km-tooltip-item">
          <div class="km-tooltip-term">${{t.term}}</div>
          <div class="km-tooltip-def">${{t.desc}}</div>
        </div>`
      ).join("");
    }}
    ...
    // Alert row
    let alertHtml = "";
    if (estado === "Parcial" && c.desfase_meses > 0) {{
      alertHtml = `<div class="km-alert yellow">⚠ A este ritmo llegás ${{c.desfase_meses}} mes${{c.desfase_meses !== 1 ? "es" : ""}} más tarde</div>`;
    }} else if (estado === "En espera") {{
      alertHtml = `<div class="km-alert red">✕ Necesitás +${{fmtNum(c.cuota_ideal, c.moneda)}}/mes para activarla</div>`;
    }}
    ...
    div.innerHTML = `
      <div class="km-card-top">
        <div style="display:flex;align-items:center;gap:7px;min-width:0;">
          <span class="km-order-num" data-order="${{orderNum}}">${{orderNum}}</span>
          <div style="min-width:0;">
            <div class="km-card-title">${{c.meta}}</div>
            <div class="km-card-sub">${{c.categoria}} · ${{c.moneda}} · ${{c.prioridad}}</div>
          </div>
        </div>
        <div class="km-prio-dot" style="background:${{prioColor}};"></div>
      </div>

      <div class="km-bar-wrap">
        <div class="km-bar-fill" style="width:${{c.pct_prog}}%;background:${{barColor}};"></div>
      </div>
      <div class="km-bar-labels">
        <span>${{c.pct_prog}}% completado</span>
        <span>${{fmtNum(c.ya_ahorrado, c.moneda)}} / ${{fmtNum(c.costo_total, c.moneda)}}</span>
      </div>

      <div class="km-stat-grid">
        <div class="km-stat">
          <div class="km-stat-lbl">Ahorro este mes</div>
          <div class="km-stat-val" style="color:${{barColor}};">${{fmtNum(c.cuota_asignada, c.moneda)}}</div>
          <div style="font-size:10px;color:${{diffColor}};margin-top:1px;">${{diffTxt}} vs ideal</div>
        </div>
        <div class="km-stat">
          <div class="km-stat-lbl">Faltante (con inflación)</div>
          <div class="km-stat-val" style="color:#c4c4c4;">${{fmtNum(c.faltante_futuro, c.moneda)}}</div>
        </div>
      </div>

      <div class="km-date-row">
        <span class="km-date-left">📅 Llegada estimada</span>
        <div style="text-align:right;">
          <div class="km-date-val">${{c.fecha_llegada}}</div>
          ${{c.desfase_meses > 0 ? `<div class="km-desfase">+${{c.desfase_meses}} meses de retraso</div>` : ""}}
        </div>
      </div>

      ${{alertHtml}}

      <button class="km-instr-toggle" onclick="toggleInstr(this)">
        <span>${{c.instrumento_emoji}} Instrumento sugerido</span>
        <span class="arrow">▼</span>
      </button>
      <div class="km-instr-body">
        <div class="km-instr-name">${{c.instrumento_tipo}}</div>
        <div class="km-instr-desc">${{c.instrumento_desc}}</div>
        ${{c.alternativas ? `<div class="km-instr-alts">Alternativas: ${{c.alternativas}}</div>` : ""}}
        ${{tooltipsHtml}}
      </div>
    `;
    return div;
  ...
  window.toggleInstr = toggleInstr;

  function updateCounts() {{
    ["En curso", "Parcial", "En espera"].forEach(estado => {{
      const drop = document.getElementById(DROP_IDS[estado]);
      const cnt  = document.getElementById(CNT_IDS[estado]);
      if (drop && cnt) cnt.textContent = drop.children.length;
    }});
    // Update order numbers
    ["drop-encurso","drop-parcial","drop-espera"].forEach(dropId => {{
      const drop = document.getElementById(dropId);
      if (!drop) return;
      Array.from(drop.children).forEach((card, i) => {{
        const num = card.querySelector("[data-order]");
  ...
  CARDS.forEach((c, i) => {{
    const dropId = DROP_IDS[c.estado] || "drop-encurso";
    const drop = document.getElementById(dropId);
    if (drop) drop.appendChild(buildCard(c, i + 1));
  }});
  updateCounts();
  ...
  document.querySelectorAll(".km-dropzone").forEach(zone => {{
  ...
  document.addEventListener("dragend", e => {{
    ...
    dragCard = null;
    document.querySelectorAll(".km-dropzone").forEach(z => z.classList.remove("drag-over"));
  }});

  function getDragAfterElement(container, y) {{
    const els = [...container.querySelectorAll(".km-card:not(.dragging)")];
...
"""
    ...
    st.components.v1.html(_kanban_html, height=max(500, len(objetivos_enriquecidos) * 260), scrolling=False)
